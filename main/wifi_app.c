...
/* 
    ...
    @param arg data, aside from the event data that is passes to the handler when it is called 
    @param event_base the abse id of the event to register the handler for 
    @param even_id to the id for the event to register the handler for 
...
/* 
    Initializes the wifi application event handler and wifi and IP events 
 */
...
/* 
    Initializes the TCP/IP stack and default wifi config
 */
static void wifi_app_deafult_wifi_init(void)
{
...
static void wifi_app_soft_ap_config(void){
    ...
    inet_pton(AF_INET,WIFI_AP_GATEWAY, &ap_ip_info.gw); //Assign access points static IP, GW and netmask
    inet_pton(AF_INET,WIFI_AP_NETMASK, &ap_ip_info.netmask); //Assign access points static IP, GW and netmask
    ...
    ESP_ERROR_CHECK(esp_wifi_set_mode(WIFI_MODE_APSTA));   //Setting the mode as access point/ station
